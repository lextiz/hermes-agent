...
def test_default_config_disables_intermediate_response_timeout() -> None:
    """The upstream default should be opt-in, not an unsolicited gateway message."""
    from hermes_cli.config import DEFAULT_CONFIG

    assert DEFAULT_CONFIG["agent"]["gateway_intermediate_response_timeout"] == 0
...
def test_config_gateway_timeout_wins_over_stale_env(hermes_home: Path) -> None:
    ...
    _write_config(hermes_home, agent_cfg={
        ...
        "gateway_intermediate_response_timeout": 10,
    })
    _write_env(hermes_home, {
        "HERMES_AGENT_TIMEOUT": "60",
        "HERMES_AGENT_TIMEOUT_WARNING": "30",
        "HERMES_AGENT_INTERMEDIATE_RESPONSE_TIMEOUT": "120",
    })
    ...
    assert env.get("HERMES_AGENT_INTERMEDIATE_RESPONSE_TIMEOUT") == "10"
